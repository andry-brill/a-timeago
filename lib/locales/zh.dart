...
const _localeName = 'zh';

/// Ten-year unit using Unicode CLDR 48.1 Chinese duration labels.
const TimeAgoStep decade = TimeAgoStep.customUnit(
  TimeAgoCustomUnitTranslation(
    unit: TimeAgoUnit.year,
    unitMultiplier: 10,
    units: TimeAgoFormatSet(
      long: TimeAgoPluralLabels.same('{0}个十年'),
      short: TimeAgoPluralLabels.same('{0}个十年'),
      narrow: TimeAgoPluralLabels.same('{0}个十年'),
    ),
  ),
);

/// Hundred-year unit using Unicode CLDR 48.1 Chinese duration labels.
const TimeAgoStep century = TimeAgoStep.customUnit(
  TimeAgoCustomUnitTranslation(
    unit: TimeAgoUnit.year,
    unitMultiplier: 100,
    units: TimeAgoFormatSet(
      long: TimeAgoPluralLabels.same('{0}个世纪'),
      short: TimeAgoPluralLabels.same('{0}个世纪'),
      narrow: TimeAgoPluralLabels.same('{0}个世纪'),
    ),
  ),
);
...
const _long = TimeAgoFormatLabels(
  relative: TimeAgoRelativeLabels(
    second: TimeAgoRelativeUnitLabels.same(
        current: '现在', past: '{0}秒钟前', future: '{0}秒钟后'),
...
const _mini = TimeAgoFormatLabels(
  relative: TimeAgoRelativeLabels(
    second: TimeAgoRelativeUnitLabels(
        past: TimeAgoPluralLabels.same('{0}秒钟前'),
        future: TimeAgoPluralLabels.same('{0}秒钟后')),
    minute: TimeAgoRelativeUnitLabels(
        past: TimeAgoPluralLabels.same('{0}分钟前'),
        future: TimeAgoPluralLabels.same('{0}分钟后')),
    hour: TimeAgoRelativeUnitLabels(
        past: TimeAgoPluralLabels.same('{0}小时前'),
        future: TimeAgoPluralLabels.same('{0}小时后')),
    day: TimeAgoRelativeUnitLabels(
        past: TimeAgoPluralLabels.same('{0}天前'),
        future: TimeAgoPluralLabels.same('{0}天后')),
    week: TimeAgoRelativeUnitLabels(
        past: TimeAgoPluralLabels.same('{0}周前'),
        future: TimeAgoPluralLabels.same('{0}周后')),
    month: TimeAgoRelativeUnitLabels(
        past: TimeAgoPluralLabels.same('{0}个月前'),
        future: TimeAgoPluralLabels.same('{0}个月后')),
    quarter: TimeAgoRelativeUnitLabels(
        past: TimeAgoPluralLabels.same('{0}个季度前'),
        future: TimeAgoPluralLabels.same('{0}个季度后')),
